        ...
        when {
            ...
            header.isDependencyListHeader -> {
                table.entries
                    .asSequence()
                    .filter {
                        (it.value as? TomlInlineTable)?.getValueWithKey("optional")?.isBooleanTrue == true
                    }
                    .map { it.key }
            }
            ...
            header.isSpecificDependencyTableHeader -> {
                if (table.getValueWithKey("optional")?.isBooleanTrue == true) {
...
private val TomlValue.isBooleanTrue: Boolean
    get() = ((this as? TomlLiteral)?.kind as? TomlLiteralKind.Boolean)?.value == true
